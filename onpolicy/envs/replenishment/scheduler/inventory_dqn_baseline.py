...
class ConsumerDQNTorchPolicy(BaselinePolicy):
  ...
  def store_transition(self, state, action, reward, next_state, done,
                       demand_hist, demand_pred, next_demand_hist, agent_id):
    ...
    next_demand_hist = torch.tensor(next_demand_hist.copy(),
                                    dtype=torch.float32)

    self.memory.push([
        state, action, reward, next_state, done, demand_hist, demand_pred,
        next_demand_hist
    ])
