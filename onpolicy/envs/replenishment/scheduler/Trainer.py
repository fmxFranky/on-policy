...
class Trainer():
  ...
  def train(self, iter):
    ...
    action_distribution = {
        key: [0 for i in range(int(self.env.action_space_consumer.n))
             ] for key in self.policies_to_train
    }
    episode_steps = []
    episode_step = 0
    uncontrollable_part_state = {
        key: infos[key][self.uncontrollable_part_state_key].copy()
        for key in self.policies_to_train
    }
    uncontrollable_part_pred = {
        key: infos[key]['uncontrollable_part_pred'].copy()
        for key in self.policies_to_train
    }
    ...
    for i in range(self.training_steps):
      ...
      for agent_id in self.policies_to_train:
        ...
        self.policies[agent_id].store_transition(
            obss[agent_id], actions[agent_id], rewards[agent_id],
            next_obss[agent_id], done, uncontrollable_part_state[agent_id],
            uncontrollable_part_pred[agent_id],
            infos[agent_id][self.uncontrollable_part_state_key], agent_id)
        uncontrollable_part_state[agent_id] = infos[agent_id][
            self.uncontrollable_part_state_key].copy()
        uncontrollable_part_pred[agent_id] = infos[agent_id][
            'uncontrollable_part_pred'].copy()
        action_distribution[agent_id][actions[agent_id]] += 1

      if self.step % (self.update_period * len(self.policies_to_train)) == 0:
        for agent_id in self.policies_to_train:
          loss, qvalue, pred_loss = self.policies[agent_id].learn(
              self.batch_size)
  ...
  def load_data(self, eval=False):
    ...
    for i in range(self.training_steps):
      ...
      for agent_id, obs in obss.items():
        policy = self.policies[agent_id]
        action, new_state, _ = policy.compute_single_action(
            obs, state=None, info=infos[agent_id], explore=True)
        actions[agent_id] = action
      next_obss, rewards, dones, infos = self.env.step(actions)
