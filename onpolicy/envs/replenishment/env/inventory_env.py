...
from ..agents.base import Agent, BalanceSheet, Cell
from ..agents.inventory import *
from ..agents.inventory import World
from ..agents.inventory_order import *
from ..config.inventory_config import (sku_config, store_config,
                                       supplier_config, warehouse_config)
from .gamma_sale_retailer_world import load_sale_sampler as gamma_sale_sampler
from .inventory_action_calculator import ActionCalculator
from .inventory_reward_calculator import RewardCalculator
from .inventory_state_calculator import StateCalculator
from .inventory_utils import Utils
from .online_retailer_world import load_sale_sampler as online_sale_sampler

initial_balance = 1000000
all_sale_sampler = [
    online_sale_sampler(
        f"{os.path.dirname(__file__)}/../data/OnlineRetail/store{i+1}_new.csv")
    for i in range(2)
]
...
class WorldBuilder():
  ...
  def create(x=80, y=32):
    ...
    for i in range(n_supplies):
      supplier_config = SupplierCell.Config(
          max_storage_capacity=Utils.get_supplier_capacity(i),
      ...
      f = SupplierCell(supplier_x, supplier_y, world, supplier_config,
    ...
    for echelon in range(n_echelon):
      ...
      for i in range(n_warehouses):
        warehouse_config = WarehouseCell.Config(
            max_storage_capacity=Utils.get_warehouse_capacity(echelon, i),
            unit_storage_cost=Utils.get_warehouse_unit_storage_cost(echelon, i),
            fleet_size=Utils.get_warehouse_fleet_size(echelon, i),
            unit_transport_cost=Utils.get_warehouse_unit_transport_cost(
                echelon, i))
        if n_warehouses > 1:
          warehouse_y = int(size_y_margins / (n_warehouses - 1) * i +
                            map_margin)
        else:
          warehouse_y = int(size_y_margins / 2 + map_margin)
        w = WarehouseCell(echelon_x, warehouse_y, world, warehouse_config,
                          default_economy_config())
        w.idx_in_config = i
        ...
        for _, sku_info in enumerate(sku_info_list):
          candidate_upstream_suppliers = pre_warehouse_sources[
              sku_info['sku_name']]
          upstream_suppliers = []
          ...
          warehouse_sku_config = ProductUnit.Config(
              sources=upstream_suppliers,
              unit_manufacturing_cost=sku_info.get('cost', 10),
              sale_gamma=sku_info.get('sale_gamma', 10),
              bill_of_materials=bom)
          sku = SKUWarehouseUnit(
              w, warehouse_sku_config,
          ...
          w.storage.try_add_units(
              {sku_info['sku_name']: sku_info.get('init_stock', 0)})
    ...
    for i in range(n_stores):
      store_config = RetailerCell.Config(
          max_storage_capacity=Utils.get_store_capacity(i),
      ...
      r = RetailerCell(retailer_x, retailer_y, world, store_config,
                       default_economy_config())
      r.idx_in_config = i
      ...
      for _, sku_info in enumerate(sku_info_list):
        candidate_upstream_warehouses = pre_warehouse_sources[
            sku_info['sku_name']]
        upstream_warehouses = []
        ...
        bom = BillOfMaterials({sku_info['sku_name']: 1}, sku_info['sku_name'])
        retail_sku_config = ProductUnit.Config(
            sources=upstream_warehouses,
            unit_manufacturing_cost=sku_info.get('cost', 10),
            sale_gamma=sku_info.get('sale_gamma', 10),
            bill_of_materials=bom)
        ...
        if Utils.get_demand_sampler() == "DYNAMIC_GAMMA":
          sku = SKUStoreUnit(
              r, retail_sku_config,
              default_economy_config(order_cost=r.facility_info['order_cost']))
        elif Utils.get_demand_sampler() == "GAMMA":
          sale_sampler = gamma_sale_sampler(i)
          sku = OuterSKUStoreUnit(
              r, retail_sku_config,
              default_economy_config(order_cost=r.facility_info['order_cost']),
              sale_sampler)
        else:
  ...
  def connect_cells(world, source, *destinations):
    for dest_cell in destinations:
      WorldBuilder.build_railroad(world, source.x, source.y, dest_cell.x,
                                  dest_cell.y)
...
class InventoryManageEnv(MultiAgentEnv):
  ...
  def __init__(self, env_config):
    ...
    self.action_space_consumer = Discrete(len(
        Utils.get_consumer_action_space()))
    ...
    self.observation_space = Box(low=-300.00,
                                 high=300.00,
                                 shape=(state_dim,),
                                 dtype=np.float64)
  ...
  def reset(self, eval=False, eval_on_trainingset=False):
    # print("Rst!")
    if (self.env_config['training'] and self.env_config['init'] == 'rnd' and
        self.copy_world):
  ...
  def step(self, action_dict):
    ...
    for _ in range(self.env_config['downsampling_rate'] - 1):
      ...
      for agent_id in balances.keys():
        ...
        outcome.facility_step_balance_sheets[
            agent_id] += nop_outcome.facility_step_balance_sheets[agent_id]

    all_done = (self.world.time_step >= self.done_step)
    # all_done = (self.world.time_step >= self.env_config['episod_duration']+20)
    # all_done = False
    if (self.env_config['training'] and self.env_config['init'] == 'rnd' and
        self.world.time_step == self.world_idx):
    ...
    seralized_states, info_states = self.state_calculator.world_to_state(
        self.world)
  ...
  def get_stock_status(self):
    ...
    for facility in self.world.facilities.values():
      if isinstance(facility, ProductUnit):
        ...
        stock_info[facility_key] = facility.storage.stock_levels.get(
            sku_name, 0)
      elif isinstance(facility, FacilityCell):
        facility_key = f"{facility.id}_{facility.idx_in_config}"
        stock_info[facility_key] = np.sum(
            list(facility.storage.stock_levels.values()))
    return stock_info
      ...
      if isinstance(facility, ProductUnit):
        ...
        if facility.consumer is not None and facility.consumer.sources is not None:
          for source in facility.consumer.sources:
            order_in_transit_info[
                facility_key] += facility.consumer.open_orders.get(
                    source.id, {}).get(sku_name, 0)
      elif isinstance(facility, FacilityCell):
        facility_key = f"{facility.id}_{facility.idx_in_config}"
        order_in_transit_info[facility_key] = 0
        for sku in facility.sku_in_stock:
          if sku.consumer is not None and sku.consumer.sources is not None:
            for source in sku.consumer.sources:
              order_in_transit_info[
                  facility_key] += sku.consumer.open_orders.get(
                      source.id, {}).get(sku.sku_info['sku_name'], 0)
