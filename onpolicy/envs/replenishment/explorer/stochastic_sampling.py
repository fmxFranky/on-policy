...
class StochasticSampling(Exploration):
  ...
  def get_exploration_action(self,
    ...
    if self.framework == "torch":
      return self._get_torch_exploration_action(action_distribution, timestep,
                                                explore)
    else:
      return self._get_tf_exploration_action_op(action_distribution, timestep,
                                                explore)
  ...
  def _get_tf_exploration_action_op(self, action_dist, timestep, explore):
    ...
    stochastic_actions = tf.cond(
        ...
        true_fn=lambda:
        (self.random_exploration.get_tf_exploration_action_op(action_dist,
                                                              explore=True)[0]),
    ...
    if self.framework in ["tf2", "tfe"]:
      ...
      assign_op = (tf1.assign_add(self.last_timestep, 1) if timestep is None
