  ...
  def _transfer_to_daily_sale(self):
    ...
    for _, row in self.df.iterrows():
      for i in range(self.period):
        ...
        daily_record_list.append([
            dt, row[self.id_col], row[self.sale_price_col],
            row[self.sale_col] * self.distribute_proportion[i]
        ])
    self.df = pd.DataFrame(
        data=daily_record_list,
        columns=[self.dt_col, self.id_col, self.sale_price_col, self.sale_col])
