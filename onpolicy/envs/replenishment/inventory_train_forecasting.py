...
def train_forecasting_model(args):
  ...
  for i in range(forecast_config['train_round']):
    eval_loss = forecast_model.eval_one_round(
        forecast_config['evaluation_steps'])
    train_loss = forecast_model.train_one_round(
        forecast_config['training_steps'])

    print(f"round {i}")
    print(
        f"train_loss: max: {np.max(train_loss):13.6f} mean: {np.mean(train_loss):13.6f} min: {np.min(train_loss):13.6f}"
    )
    print(
        f"eval_loss:  max: {np.max(eval_loss):13.6f} mean: {np.mean(eval_loss):13.6f} min: {np.min(eval_loss):13.6f}"
    )
...
parser.add_argument("--run-name",
                    type=str,
                    default='1223_forecasting_hid32_sigmoid_2layer_train_more')
