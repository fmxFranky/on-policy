...
class Scenario(BaseScenario):
  ...
  def agent_reward(self, agent, world):
    ...
    if shaped_adv_reward:  # distance-based adversary reward
      ...
      adv_rew = 0
      for a in adversary_agents:
        if np.sqrt(np.sum(np.square(a.state.p_pos -
                                    a.goal_a.state.p_pos))) < 2 * a.goal_a.size:
          adv_rew -= 5
  ...
  def adversary_reward(self, agent, world):
    ...
    if shaped_reward:  # distance-based reward
      ...
      adv_rew = 0
      if np.sqrt(np.sum(
          np.square(agent.state.p_pos -
                    agent.goal_a.state.p_pos))) < 2 * agent.goal_a.size:
        adv_rew += 5
      return adv_rew
  ...
  def observation(self, agent, world):
    ...
    if not agent.adversary:
      return np.concatenate([agent.goal_a.state.p_pos - agent.state.p_pos] +
