...
def color_char_to_idx(color_char):
  ...
  """
    ...
    try:
        return next(
            idx for (idx, c) in enumerate(COLOR_CHAR) if c == color_char)
    ...
    """
        if params is None:
            ...
            for key in params:
                ...
                param_list.append(
                    ffi.new("char[]",
                            str(params[key]).encode('ascii')))
            c_array = ffi.new("char * [" + str(len(param_list)) + "]",
                              param_list)
            self._game = ffi.new("pyhanabi_game_t*")
    ...
    """
        ...
        for pid in range(self.num_players()):
            ...
            for i in range(hand_size):
                ...
                lib.ObsGetHandCardKnowledge(self._observation, pid, i,
                                            c_knowledge)
