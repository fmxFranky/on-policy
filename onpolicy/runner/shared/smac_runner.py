...
class SMACRunner(Runner):
      ...
      if episode % self.log_interval == 0:
        ...
        if self.env_name == "StarCraft2":
          ...
          incre_win_rate = np.sum(incre_battles_won) / np.sum(
              incre_battles_game) if np.sum(incre_battles_game) > 0 else 0.0
  ...
  def insert(self, data):
    ...
    rnn_states[dones_env == True] = np.zeros(
        ((dones_env
          == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size),
        dtype=np.float32)
  ...
  def eval(self, total_num_steps):
    ...
    while True:
      ...
      eval_rnn_states[eval_dones_env == True] = np.zeros(
          ((eval_dones_env == True).sum(), self.num_agents, self.recurrent_N,
           self.hidden_size),
          dtype=np.float32)
