...
class HanabiRunner(Runner):
  ...
  def run(self):
    ...
    for episode in range(episodes):
      ...
      for step in range(self.episode_length):
        ...
        self.buffer.chooseinsert(self.turn_share_obs, self.turn_obs,
                                 self.turn_rnn_states,
                                 self.turn_rnn_states_critic, self.turn_actions,
                                 self.turn_action_log_probs, self.turn_values,
                                 self.turn_rewards, self.turn_masks,
                                 self.turn_bad_masks, self.turn_active_masks,
      ...
      if episode % self.log_interval == 0 and episode > 0:
        ...
        print(
            ...
            .format(self.all_args.hanabi_name, self.algorithm_name,
                    self.experiment_name, episode, episodes, total_num_steps,
                    self.num_env_steps, int(total_num_steps / (end - start))))
        ...
        if self.env_name == "Hanabi":
          ...
          if self.use_wandb:
            ...
            self.writter.add_scalars('average_score',
  ...
  def collect(self, step):
    for current_agent_id in range(self.num_agents):
      ...
      self.turn_share_obs[choose,
                          current_agent_id] = self.use_share_obs[choose].copy()
      self.turn_available_actions[
          choose, current_agent_id] = self.use_available_actions[choose].copy()
      ...
      self.turn_rnn_states_critic[choose,
                                  current_agent_id] = _t2n(rnn_state_critic)
      ...
      self.turn_rewards[choose,
                        current_agent_id] = self.turn_rewards_since_last_action[
