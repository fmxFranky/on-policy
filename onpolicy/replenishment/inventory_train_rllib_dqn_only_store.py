...
policies = {
    ...
    'baseline_consumer': (ConsumerBaselinePolicy, env.observation_space,
                          ...
                          BaselinePolicy.get_config_from_env(env)),
    'dqn_store_consumer':
        (DQNTorchPolicy, env.observation_space, env.action_space_consumer,
         dqn_policy_config_store_consumer),
}
...
def train_dqn(args):
  ...
  dqn_trainer = dqn.DQNTrainer(env=InventoryManageEnv, config=ext_conf)

  max_mean_reward = -100

  for i in range(args.stop_iters):
    print("== Iteration", i, "==")
    dqn_trainer.workers.foreach_worker(lambda ev: ev.foreach_env(
...
parser.add_argument("--run-name",
                    type=str,
                    default='dqn_rllib_test1_rbaseline_debug')
