...
from ray.rllib.models.torch.torch_action_dist import \
    TorchMultiCategorical as MultiCategorical
...
from scheduler.inventory_minmax_policy import \
...
from scheduler.inventory_torch_model import \
    SKUStoreBatchNormModel as SKUStoreDNN
from scheduler.inventory_torch_model import \
    SKUWarehouseBatchNormModel as SKUWarehouseDNN
...
policies = {
    'baseline_producer': (ProducerBaselinePolicy, env.observation_space,
                          env.action_space_producer,
                          BaselinePolicy.get_config_from_env(env)),
    'baseline_consumer': (ConsumerBaselinePolicy, env.observation_space,
                          env.action_space_consumer,
                          BaselinePolicy.get_config_from_env(env)),
    'ppo_producer': (MyTorchPolicy, env.observation_space,
                     env.action_space_producer, ppo_policy_config_producer),
    'ppo_store_consumer':
        (MyTorchPolicy, env.observation_space, env.action_space_consumer,
         ppo_policy_config_store_consumer),
    'ppo_warehouse_consumer':
        (MyTorchPolicy, env.observation_space, env.action_space_consumer,
         ppo_policy_config_warehouse_consumer)
}
...
def print_training_results(result):
  keys = [
      'date', 'episode_len_mean', 'episodes_total', 'episode_reward_max',
      'episode_reward_mean', 'episode_reward_min', 'timesteps_total',
      'policy_reward_max', 'policy_reward_mean', 'policy_reward_min'
  ]
  for k in keys:
    print(f"- {k}: {result[k]}")
...
def create_ppo_trainer(echelon):
  ...
  policies_to_train = (['ppo_store_consumer']
                       if echelon == env.world.total_echelon -
                       1 else ['ppo_warehouse_consumer'])
  ...
  ext_conf.update({
      ...
      "exploration_config": {
          ...
          "random_timesteps":
              10000,  # args.rollout_fragment_length*args.batch_size*args.stop_iters // 2,
      },
      "multiagent": {
          "policies":
              filter_keys(policies, [
                  'baseline_producer', 'baseline_consumer',
                  'ppo_store_consumer', 'ppo_warehouse_consumer'
              ]),
          "policy_mapping_fn":
              policy_map_fn,
          "policies_to_train":
...
if __name__ == "__main__":
  ...
  for echelon in range(total_echelon - 2, total_echelon - echelon_to_train - 1,
                       -1):
    ppo_trainer = create_ppo_trainer(echelon)
    ppo_trainer.set_weights(
        retailer_ppo_trainer.get_weights('ppo_store_consumer'))
    ...
    pre_warehouse_trainer_weight = ppo_trainer.get_weights(
        'ppo_warehouse_consumer')

  # Create the environment
  evaluation_epoch_len = 60
  env.set_iteration(1, 1)
  env.env_config.update({
      'episod_duration': evaluation_epoch_len,
      'downsampling_rate': 1
  })
  print(
