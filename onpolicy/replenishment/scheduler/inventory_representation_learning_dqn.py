...
class mixed_dqn_net(nn.Module):
  ...
  def __init__(self, config):
    ...
    if config['activation_func'] == 'relu':
      ...
      raise Exception(
          "embedding merge error, three legal option 'relu', 'sigmoid' or 'tanh'"
      )

    self.embedding_merge = config['embeddingmerge']
    if self.embedding_merge == 'cat':
      self.dqn_fc1 = nn.Linear(self.dqn_embedding_size * 2,
                               self.dqn_hidden_size)
    ...
    self.controllable_embed_fc1 = nn.Linear(self.controllable_state_num,
                                            self.dqn_embedding_size)
    ...
    self.uncontrollable_embed_fc1 = nn.Linear(self.uncontrollable_state_num,
                                              self.uncontrollable_hidden_size)
    self.uncontrollable_embed_fc2 = nn.Linear(self.uncontrollable_hidden_size,
                                              self.dqn_embedding_size)

    self.uncontrollable_embed_bn1 = nn.BatchNorm1d(
        self.uncontrollable_hidden_size)
    self.uncontrollable_embed_bn2 = nn.BatchNorm1d(self.dqn_embedding_size)

    self.uncontrollable_pred = nn.Linear(self.dqn_embedding_size,
                                         self.uncontrollable_pred_num)
  ...
  def forward(self, controllable_state, uncontrollable_state=None):
    ...
    if uncontrollable_state != None:
      ...
      if self.embedding_merge == 'cat':
        ...
        raise Exception(
            "embedding merge error, two legal option 'cat' or 'dot'")
...
class mixed_dqn_unc_cnn_net(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.uncontrollable_embed_conv1 = nn.Conv1d(in_channels=7,
                                                out_channels=16,
                                                kernel_size=3,
                                                padding=1)
    self.uncontrollable_embed_conv2 = nn.Conv1d(in_channels=16,
                                                out_channels=32,
                                                kernel_size=3,
                                                padding=1)
  ...
  def __uncontrollable_embedding(self, uncontrollable_state):
    x = self.uncontrollable_embed_conv1(
        uncontrollable_state)  # batch x 5 x 21 -> batch x 16 x 21
    x = x.permute(0, 2, 1)
    if self.use_bn:
      x = self.uncontrollable_embed_bn1(x)
    x = x.permute(0, 2, 1)
...
class ConsumerRepresentationLearningDQNTorchPolicy(BaselinePolicy):
  ...
  def store_transition(self,
    ...
    if eval:
      self.eval_memory.push([
          state, action, reward, next_state, done, unc_state, unc_pred,
          next_unc_state
      ])
    else:
      self.memory.push([
  ...
  def learn(self, batch_size):
    ...
    self.epsilon = 0.999 * self.epsilon + 0.001 * self.dqn_config[
        'epsilon_train']
    ...
    batch_next_unc_state = self.demand_augmentation.augment(
        batch_next_unc_state)
    ...
    with torch.no_grad():
      ...
      q_target = batch_reward + self.dqn_config['gamma'] * (1 -
                                                            batch_done) * q_next
  ...
  def eval_embedding(self, batch_size=1024):
    ...
    with torch.no_grad():
      unc_pred_from_net = self.eval_net.uncontrollable_pred_head(
          batch_unc_state)
      loss = self.loss_func(unc_pred_from_net, batch_unc_pred)
    return loss.item()
  ...
  def pre_train(self, writer):
    ...
    for i in range(self.dqn_config['pretrain_epoch']):
      ...
      print(
          f"pretraining epoch {i}, before train eval loss {eval_loss} train loss {train_loss}"
      )
