...
class SKUStoreDNN(FullyConnectedNetwork, nn.Module):
  ...
  def __init__(self, obs_space, action_space, num_outputs, model_config, name):
    ...
    self._hidden_layers = nn.Sequential(
        nn.Linear(self.obs_size, 2 * self.hidden_dim), nn.ReLU(),
        nn.Linear(2 * self.hidden_dim, self.hidden_dim))
    self._logits = nn.Sequential(nn.ReLU(),
  ...
  def forward(self, input_dict, state, seq_lens):
    ...
    assert torch.sum(torch.isnan(list(
        self.parameters())[0].data)).item() == 0, 'weights'
    assert torch.sum(torch.isinf(list(
        self.parameters())[0].data)).item() == 0, 'weights inf'
    return logits, state
...
class SKUStoreGRU(RecurrentNetwork, nn.Module):
  ...
  def __init__(self, obs_space, action_space, num_outputs, model_config, name):
    ...
    self.fc1 = nn.Sequential(
        nn.Linear(self.obs_size, 2 * self.rnn_hidden_dim), nn.ReLU(),
    ...
    self.fc2 = nn.Sequential(nn.ReLU(),
                             nn.Linear(self.rnn_hidden_dim, num_outputs))
...
class SKUWarehouseNet(RecurrentNetwork, nn.Module):
  ...
  def __init__(self, obs_space, action_space, num_outputs, model_config, name):
    ...
    self.rnn_hidden_dim = model_config["lstm_cell_size"]
    self.fc1 = nn.Sequential([
        nn.Linear(self.obs_size, 2 * self.rnn_hidden_dim),
        nn.ReLU(),
        nn.Linear(2 * self.rnn_hidden_dim, self.rnn_hidden_dim)
    ])
    self.rnn = nn.GRU(self.rnn_hidden_dim, self.rnn_hidden_dim, 1)
    self.fc2 = nn.Sequential(
        [nn.ReLU(), nn.Linear(self.rnn_hidden_dim, num_outputs)])
    self.value_branch = nn.Linear(self.rnn_hidden_dim, 1)
    self._cur_value = None
