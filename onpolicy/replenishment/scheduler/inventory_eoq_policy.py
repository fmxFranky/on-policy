...
from .inventory_random_policy import ConsumerBaselinePolicy
...
class ConsumerEOQPolicy(ConsumerBaselinePolicy):
  ...
  def __init__(self, observation_space, action_space, config):
    ConsumerBaselinePolicy.__init__(self, observation_space, action_space,
                                    config)
  ...
  def _get_consumer_quantity(self, f_state_info):
    ...
    consumer_quantity = int(
        np.sqrt(2 * sale_gamma * order_cost / holding_cost) / sale_gamma)
  ...
  def _find_source(self, f_state_info):
    ...
    if most_needed_product_id is not None:
      for i in range(self.n_sources):
        for j in range(self.n_products):
          if f_state_info['consumer_source_export_mask'][i * self.n_products +
                                                         j] == 1:
    ...
    vlt_buffer_days = 7 + 5 * (Utils.get_env_config()['total_echelons'] -
                               f_state_info['echelon_level'])
    vlt = vlt_buffer_days + f_state_info['vlt'][source_id * self.n_products +
    ...
    if booked_inventory[most_needed_product_id] > vlt * sale_mean + np.sqrt(
        vlt) * sale_std * st.norm.ppf(service_level):
      return 0
