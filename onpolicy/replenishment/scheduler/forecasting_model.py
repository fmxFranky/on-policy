...
class Forecasting_model():
  ...
  def load_data(self, pr=False, name='test'):
    ...
    for i in range(2):
      ...
      sale_sampler = online_sale_sampler(
        ...
        for k in range(self.train_len + self.validation_len):
          ...
          if pr:
            ...
            if data_info['year'] == 2016 and k < 1938:
              ...
              writer_pred.add_scalar(f'zzeval/demand_s{i}_{key}', _pred,
                                     data_info['dayofyear'])
              writer_true.add_scalar(f'zzeval/demand_s{i}_{key}', _true,
                                     data_info['dayofyear'])
      ...
      for key, demand in sale_sampler.sale_ts_cache.items():
        for k in range(len(demand)):
          ...
          if date_info['year'] >= 2012 and date_info['year'] <= 2015:
            ...
            writer[index].add_scalar(f'zzz/demand_s{i}_{key}', demand[k],
                                     date_info['dayofyear'])
