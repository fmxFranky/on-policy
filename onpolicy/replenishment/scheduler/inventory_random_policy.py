...
class BaselinePolicy(Policy):
  ...
  def compute_actions(self,
    ...
    if info_batch is None:
      ...
      action_dict = [
          self._action(f_state, f_state_info)
          for f_state, f_state_info in zip(obs_batch, info_batch)
      ], [], {}
...
class ProducerBaselinePolicy(BaselinePolicy):
  ...
  def _action(self, facility_state, facility_state_info):
    ...
    if facility_state_info is not None and len(facility_state_info) > 0:
      unit_price = self.unit_prices[np.flatnonzero(
          facility_state_info['facility_type'])[0]]
...
class ConsumerBaselinePolicy(BaselinePolicy):
  ...
  def _action(self, facility_state, facility_state_info):
    ...
    if facility_state_info is not None and len(facility_state_info) > 0:
      if np.count_nonzero(facility_state_info['bom_inputs']) > 0:
        action = default_facility_control(
            self._find_source(facility_state_info))
      else:
        action = default_facility_control(0)
    return action
  ...
  def _find_source(self, f_state_info):
    ...
    if booked_inventory[most_neeed_product_id] / f_state_info[
        'storage_capacity'] >= 1.0 / len(f_state_info['storage_levels']):
      return 0
