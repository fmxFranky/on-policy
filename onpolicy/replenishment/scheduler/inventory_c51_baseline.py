...
class ConsumerC51TorchPolicy(BaselinePolicy):
  ...
  def learn(self, batch_size):
    ...
    batch_action = batch_action.unsqueeze(1).unsqueeze(1).expand(
        batch_action.size(0), 1, self.atoms)
    dist_pred = self.eval_net(batch_state).gather(1, batch_action).squeeze(1)
    dist_true = self.projection_distribution(batch_next_state, batch_reward,
                                             batch_done)
