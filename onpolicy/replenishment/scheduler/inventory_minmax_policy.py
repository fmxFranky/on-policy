...
class ConsumerMinMaxPolicy(ConsumerBaselinePolicy):
  ...
  def _find_source(self, f_state_info):
    ...
    vlt_buffer_days = 10 + 7 * (Utils.get_env_config()['total_echelons'] -
                                f_state_info['echelon_level'])
    vlt = f_state_info['vlt'][source_id * self.n_products +
                              most_needed_product_id] + vlt_buffer_days
    ...
    R = 3 * r
    consumer_quantity = Utils.get_consumer_quantity_action(
        int((R - r) / sale_mean))
