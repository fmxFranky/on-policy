...
from scheduler.inventory_representation_learning_dqn import \
    ConsumerRepresentationLearningDQNTorchPolicy
...
def print_result(result, writer, iter):
  ...
  for agent_id, rewards in rewards_all.items():
    ...
    if tmp:
      ...
      all_action_distribution = [
          all_action_distribution[i] + action_distribution[agent_id][i]
          for i in range(9)
      ]
  print(
      f"all_step: {result['all_step']}, average episode step: {result['episode_step']}"
  )
  print(
      f"step_reward    max: {np.max(all_reward):13.6f} mean: {np.mean(all_reward):13.6f} min: {np.min(all_reward):13.6f}"
  )
  print(
      f"episode_reward max: {np.max(all_episode_reward):13.6f} mean: {np.mean(all_episode_reward):13.6f} min: {np.min(all_episode_reward):13.6f}"
  )
  print(
      f"loss           max: {np.max(all_loss):13.6f} mean: {np.mean(all_loss):13.6f} min: {np.min(all_loss):13.6f}"
  )
  print(
      f"pred loss      max: {np.max(all_pred_loss):13.6f} mean: {np.mean(all_pred_loss):13.6f} min: {np.min(all_pred_loss):13.6f}"
  )
  print(
      f"qvalue         max: {np.max(all_qvalue):13.6f} mean: {np.mean(all_qvalue):13.6f} min: {np.min(all_qvalue):13.6f}"
  )
  ...
  for i in range(9):
    writer.add_scalar(f'action/{i}', all_action_distribution[i] / sum_action,
                      iter)
...
def print_eval_result(result, writer, iter):
  ...
  for agent_id, rewards in rewards_all.items():
    ...
    if tmp:
      ...
      if all_reward == []:
        ...
        all_episode_reward = [
            a + b
            for a, b in zip(all_episode_reward, episode_reward_all[agent_id])
        ]
  ...
  writer.add_scalar('train/eval_episode_reward', np.mean(all_episode_reward),
                    iter)
...
def print_eval_on_trainingset_result(result, writer, iter):
  ...
  writer.add_scalar('train/eval_on_trainingset_step_reward',
                    np.mean(all_reward), iter)
  writer.add_scalar('train/eval_on_trainingset_episode_reward',
                    np.mean(all_episode_reward), iter)
...
parser.add_argument("--fixed-uncontrollable-param",
                    default=False,
                    type=bool_map)
parser.add_argument("--use-cnn-state", default=False, type=bool_map)
parser.add_argument("--pretrain-epoch", default=300, type=int)
parser.add_argument("--embeddingmerge",
                    default='cat',
                    type=str,
                    help="'cat' or 'dot'")
parser.add_argument("--activation-func",
                    default='sigmoid',
                    type=str,
                    help="'sigmoid', 'relu' or 'tanh'")
...
parser.add_argument("--train-augmentation",
                    default='none',
                    type=str,
                    help="pretrain and train augmentation")
