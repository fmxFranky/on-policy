...
class Utils:
  ...
  def ascii_progress_bar(done, limit, bar_lenght_char=15):
    ...
    bar = ['='] * (done_chars)
    return ''.join(bar + (['-'] * (bar_lenght_char - done_chars)) +
                   [f" {done}/{limit}"])
...
class AsciiWorldStatusPrinter():
  ...
  def status(facility: FacilityCell) -> list:
    ...
    if facility.distribution is not None:
      ...
      transport_status = [
          f"{AsciiWorldStatusPrinter.status(t)} {Utils.ascii_progress_bar(t.location_pointer, t.path_len()-1, 5)}"
          for t in u.fleet[:5]
      ]
    ...
    substatuses.append(
        ["Storage:",
         AsciiWorldStatusPrinter.status(facility.storage)])
  ...
  def status(storage: StorageUnit) -> list:
    return [
        f"Usage: {Utils.ascii_progress_bar(int(storage.used_capacity()), int(storage.max_capacity))}",
  ...
  def status(sku: ProductUnit) -> list:
    ...
    if sku.consumer is not None:
      ...
      in_transit_units_total = sum(
          sum(sku.consumer.open_orders.values(), Counter()).values())
      outbound_orders = [
          f"{src} -> {AsciiWorldStatusPrinter.counter(order)}"
          for src, order in sku.consumer.open_orders.items()
      ]
      substatuses.append([
          f"Outbound orders ({in_transit_units_total} units):", outbound_orders
      ])
      substatuses.append([
          f"Total units purchased: {sku.consumer.economy.total_units_purchased}"
      ])
      substatuses.append([
          f"Total units received: {sku.consumer.economy.total_units_received}"
      ])
      status.append(substatuses)
    if sku.seller is not None:
      substatuses = ["sku seller"]
      substatuses.append(
          [f"Current unit price: ${sku.seller.economy.unit_price}"])
      substatuses.append([f"Current demand: {sku.seller.economy.latest_sale}"])
      substatuses.append(
          [f"Total units sold: {sku.seller.economy.total_units_sold}"])
      status.append(substatuses)
    return status
...
class AsciiWorldRenderer(WorldRenderer):
  ...
  def render(self, world):
    ...
    color_theme = [
        '#000000',  # pale blue (infrastructure - backgroud)
    ...
    for layer, color in zip(ascii_layers, color_theme):
      ...
      canvas.multiline_text((margin_side, margin_top),
                            text,
                            font=font,
                            fill=color)
