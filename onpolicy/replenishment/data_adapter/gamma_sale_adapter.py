...
from .base_sale_adapter import BaseSaleAdapter
...
class GammaSaleAdapter(BaseSaleAdapter):
  ...
  def sample_sale_and_price(self, id_val, gap):
    return (self.sale_ts_cache[id_val][gap],
            self.sale_price_ts_cache[id_val][gap])
  ...
  def cache_data(self):
    ...
    for id_val in id_list:
      ...
      sale_cache_tmp = df_tmp.set_index(f"{self.dt_col}_str").to_dict('dict')[
          self.sale_col]
      sale_price_cache_tmp = df_tmp.set_index(f"{self.dt_col}_str").to_dict(
          'dict')[self.sale_price_col]
      dt_tmp = dt_min
  ...
  def _read_df(self):
    ...
    df = pd.DataFrame(
        data_list,
        columns=[self.id_col, self.dt_col, self.sale_col, self.sale_price_col])
    df.to_csv(file_name)
    return df
