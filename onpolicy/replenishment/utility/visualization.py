...
def visualization(env, policies, iteration, policy_mode):
  ...
  starter_step = env.env_config['episod_duration'] + env.env_config[
      'tail_timesteps']
  ...
  print(
      f"  === evaluation length {evaluation_epoch_len}, it will take about 1 min ....",
      flush=True)
  ...
  for epoch in range(evaluation_epoch_len):
    ...
    for agent_id, obs in obss.items():
      ...
      action, new_state, _ = policy.compute_single_action(
          obs, state=rnn_states[agent_id], info=infos[agent_id], explore=False)
    ...
    tracker.add_sku_status(0, epoch, stock_status, order_in_transit_status,
                           demand_status)
