...
from ..config.inventory_config import (demand_sampler, env_config, sku_config,
                                       store_config, supplier_config,
                                       warehouse_config)
...
class Utils:
  ...
  def get_reward_discount(inventory_level, is_retailer, echelon):
    ...
    if not is_retailer:
      inventory_level_bounds = [
          b + 2 * (total_echelon - echelon) for b in inventory_level_bounds
      ]
  ...
  def get_consumer_action_space():
    ...
    consumer_quantity_space = [
        0, 0.33, 0.66, 1, 1.33, 1.66, 2, 2.5, 3, 4, 5, 6, 7, 9, 12
    ]
  ...
  def get_supplier_info(supplier_idx):
    assert supplier_idx < Utils.get_supplier_num(
    ), "supplier_idx must be less than total supplier number"
  ...
  def get_supplier_of_sku(sku_name):
    assert (sku_name
            in Utils.get_all_skus()), f"sku must be in {Utils.get_all_skus()}"
    supplier_list = []
  ...
  def get_warehouse_info(i, warehouse_idx):
    assert warehouse_idx < Utils.get_warehouse_num(
        i), "warehouse_idx must be less than total warehouse number"
  ...
  def get_store_info(store_idx):
    assert store_idx < Utils.get_store_num(
    ), "store_idx must be less than total store number"
