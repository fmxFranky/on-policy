...
class StateCalculator:
  ...
  def _add_global_features(self, state, world):
    state[
        'global_time'] = world.time_step  # / self.env.env_config['episod_duration']
  ...
  def _add_uncontrollable_part_feature(self, state, _facility):
    state['uncontrollable_part_state'] = [
        0
    ] * self.env.env_config['uncontrollable_part_state_len']
    state['uncontrollable_part_pred'] = [
        0
    ] * self.env.env_config['uncontrollable_part_pred_len']
    ...
    if isinstance(_facility, OuterSKUStoreUnit):
      ...
      demand_future = [
          x / data_info['demand_mean'] for x in data_info['demand_pred']
      ]
      ...
      demand_history = [
          x / data_info['demand_mean'] for x in data_info['demand_hist']
      ]
      ...
      data_in.extend(
          [data_info['day'], data_info['month'], data_info['isweekend']])
      ...
      for i in range(21):
        ...
        state['uncontrollable_part_state_cnn'][1][i] = np.sin(
            date_info_hist[i]['isoweekday'] / 7 * 2 * np.pi)
        state['uncontrollable_part_state_cnn'][2][i] = np.cos(
            date_info_hist[i]['isoweekday'] / 7 * 2 * np.pi)
        state['uncontrollable_part_state_cnn'][3][i] = np.sin(
            date_info_hist[i]['month'] / 12 * 2 * np.pi)
        state['uncontrollable_part_state_cnn'][4][i] = np.cos(
            date_info_hist[i]['month'] / 12 * 2 * np.pi)
        state['uncontrollable_part_state_cnn'][5][i] = np.sin(
            date_info_hist[i]['day'] / 31 * 2 * np.pi)
        state['uncontrollable_part_state_cnn'][6][i] = np.cos(
            date_info_hist[i]['day'] / 31 * 2 * np.pi)
      state['uncontrollable_part_state'] = state[
          'uncontrollable_part_state_cnn'].copy().flatten()
  ...
  def _add_facility_features(self, state, _facility):
    ...
    if isinstance(_facility, FacilityCell):
      ...
      state['sku_info'] = {}
      state['is_positive_balance'] = 1 if _facility.economy.total_balance.total(
      ) > 0 else 0
    else:
      state['facility_info'] = _facility.facility.facility_info
      state['sku_info'] = _facility.sku_info
      state[
          'is_positive_balance'] = 1 if _facility.facility.economy.total_balance.total(
          ) > 0 else 0
  ...
  def _add_vlt_features(self, state, _facility):
    state['vlt'] = [0] * (self.env.max_sources_per_facility *
                          self.env.n_products())
    state['max_vlt'] = 0
    ...
    for i, source in enumerate(sku.consumer.sources):
      for j, product_id in enumerate(self.env.product_ids):
        if source.bom.output_product_id == product_id:
          state['vlt'][i * self.env.n_products() + j] = source.sku_info.get(
              'vlt', 2)
  ...
  def _add_sale_features(self, state, _facility):
    ...
    state['consumption_hist'] = [0
                                ] * self.env.env_config['consumption_hist_len']
    if not isinstance(_facility, ProductUnit):
      return
  ...
  def _add_consumer_features(self, state, _facility):
    ...
    state['consumer_source_export_mask'] = [0] * (
        self.env.max_sources_per_facility * self.env.n_products())
    ...
    for i, source in enumerate(sku.consumer.sources):
      for j, product_id in enumerate(self.env.product_ids):
        if source.bom.output_product_id == product_id:
          state['consumer_source_export_mask'][i * self.env.n_products() +
                                               j] = 1

    for i, product_id in enumerate(self.env.product_ids):
      # skus in stock
      state['consumer_source_inventory'][
          i] += sku.facility.storage.stock_levels.get(product_id, 0)
      for _sku in sku.facility.sku_in_stock:
        for source_id in _sku.consumer.open_orders:
          ...
          state['consumer_in_transit_orders'][i] += _sku.consumer.open_orders[
              source_id].get(product_id, 0)

    # in stock
    state['inventory_in_stock'] = sku.facility.storage.stock_levels.get(
        sku.sku_info['sku_name'], 0)
    # in transit
    state['inventory_in_transit'] = state['consumer_in_transit_orders'][
        self.env.product_ids.index(sku.sku_info['sku_name'])]
    # to be delivered to downstreaming facilities
    if sku.facility.distribution is not None:
      state[
          'inventory_in_distribution'] = sku.facility.distribution.get_pending_order(
    ...
    state['inventory_rop'] = (state['max_vlt'] * state['sale_mean'] +
                              np.sqrt(state['max_vlt']) * state['sale_std'] *
                              st.norm.ppf(state['service_level']))
  ...
  def _serialize_state(self, state):
    ...
    keys_in_state = [
        (None, [
            'is_positive_balance', 'is_over_stock', 'is_out_of_stock',
            'is_below_rop', 'echelon_level'
        ]),
        (
            'sale_mean',
            [
