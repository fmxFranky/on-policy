...
class SharedReplayBuffer(object):
  ...
  def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
    ...
    self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads,
  ...
  def compute_returns(self, next_value, value_normalizer=None):
    ...
    if self._use_proper_time_limits:
        ...
        for step in reversed(range(self.rewards.shape[0])):
          if self._use_popart:
            ...
            delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - \
                    self.value_preds[step]
            gae = delta + self.gamma * self.gae_lambda * self.masks[step +
                                                                    1] * gae
            gae = gae * self.bad_masks[step + 1]
      ...
      if self._use_gae:
        ...
        self.returns[-1] = next_value
        for step in reversed(range(self.rewards.shape[0])):
          self.returns[step] = self.returns[step + 1] * self.gamma * self.masks[
  ...
  def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
    ...
    if len(self.share_obs.shape) > 4:
      share_obs = self.share_obs[:-1].transpose(1, 2, 0, 3, 4, 5).reshape(
    ...
    for indices in sampler:
      ...
      for index in indices:
        ...
        if self.available_actions is not None:
          available_actions_batch.append(available_actions[ind:ind +
                                                           data_chunk_length])
